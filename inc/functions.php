...
function unread_notification()
{
	global $conn;
	$count = 0;
	$sql = "SELECT * FROM notifications WHERE dev_id = '".$_SESSION["developer"]["id"]."' ORDER BY id DESC LIMIT 0,10";
	$res = $conn->query($sql);
...
function insert_notes($task_id,$dev_id,$text)
{
	global $conn;
...
function send_notification($task_id,$mag_id,$text)
{
	global $conn;
...
function get_manager_name($mag_id)
{
	global $conn;
	
	$sql = "SELECT * FROM manager WHERE id = '$mag_id'";
	$res = $conn->query($sql);
	$row = mysqli_fetch_assoc($res);
	return $row['name'];
}
...
function Get_Notes($task_id,$dev_id)
{
	global $conn;
	
	$sql = "SELECT * FROM tasks_notes WHERE task_id = '$task_id' AND dev_id = '$dev_id' AND mag_id = '' ORDER BY id DESC";
	$res = $conn->query($sql);
...
function Get_Chat($task_id,$mag_id,$dev_id)
{
	...
	$sql1  = "SELECT * FROM developers WHERE id = '$dev_id'";
	...
	while($row = mysqli_fetch_assoc($res))
	{
		...
		?>
			<div class="chat_left_round"><?php echo substr(trim(get_manager_name($mag_id)), 0, 1); ?></div>
			<div class="chat_left">
				<div class="head">
					<?php echo $row['text'] ?>
				</div>
				<div class="bottom">
					<?php echo trim(get_manager_name($mag_id)).', '.date_format(date_create($row['time']),'m/d/Y, H:i A') ?>
				</div>
			</div>
			<div style="clear:both"></div>
		<?php
	...
	if(mysqli_num_rows($res) > 0){
		while($row = mysqli_fetch_assoc($res)){
            ...
            <tr onclick="notification_click('<?php echo $row["id"] ?>','<?php echo $row["task_id"] ?>')" style="<?php echo $color ?>">
                ...
                <td style="width:50%">
                    <?php
                    if(strpos($row['text'], "assigned to you") == true){
                        echo "Task assigned to you";
                    }
                    ...
                    elseif(strpos($row['text'], "to archives") == true){
...
function get_task_activity($task_id)
{
	global $conn;
	
	$sql  = "SELECT * FROM notifications WHERE dev_id = '".$_SESSION["developer"]["id"]."' AND task_id = '$task_id' ORDER BY id DESC LIMIT 0,1";
	$res = $conn->query($sql);
    $row = mysqli_fetch_assoc($res);
	if(mysqli_num_rows($res) > 0){
...
function get_task_notification($task_id,$mag_id)
{
	global $conn;
	
	$sql  = "SELECT * FROM notifications WHERE mag_id = '$mag_id' AND task_id = '$task_id' ORDER BY id DESC LIMIT 0,1";
	$res = $conn->query($sql);
    $row = mysqli_fetch_assoc($res);
	if(mysqli_num_rows($res) > 0){
...
function mail_for_change_password($user_id,$user_email)
{
	...
	$to 			=	$user_email;
	$subject		=	'Password Recovery For Developer Login';
