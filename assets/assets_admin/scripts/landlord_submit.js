...
//landlord Rental Form end
        ...
        $("body").on("blur", "#initials", function(e){
            ...
            $.ajax({
                    ...
                    success: function(response)
                    {
                        ...
                        if(jsonData[1] == 'success'){
                            ...
                            $('#result_message').html(_html);
                            $('html, body').animate({ scrollTop: $("#result_message").offset().top }, 500);
							setTimeout(function(){location.href="landlords_submit_thankyou.php"} , 5000);
                        }else{
         ...
         $("#ss-submit-broker").click(function(e)
		 {
			broker_name 	= $.trim($('#broker_name').val());
			broker_initials = $.trim($('#broker_initials').val());
			...
			if(broker_initials == ''){
				$('#broker_initials_error').css('display','block');
				return;
			}
			else{
				$('#broker_initials_error').css('display','none');
			}
			
			$("#ss-submit-broker").prop('disabled', true);
			$("#ss-submit-broker").prop('value', 'Submiting...');
			postData	 =	'';
			postData	+=	'broker_name='+broker_name;
			postData	+=	'&broker_initials='+broker_initials;
			postData	+=	'&initials='+initials;
			postData	+=	'&action=broker_landlord_agreement';
			$.ajax({
					...
					error: function(xhr, status, error)
					{
						$("#ss-submit-broker").removeAttr('disabled');
						console.log('status: '+status);
						console.log('error: '+error);
					...
					success: function(response)
					{
						$("#ss-submit-broker").html('Submitted');
						jsonData = $.parseJSON(response);
						if(jsonData[1] == 'success'){
							_html =	"<div class='alert alert-success'><button aria-hidden='true' data-dismiss='alert' class='close' type='button'></button>"+jsonData[0]+"</div>";
							$('#result_message').html(_html);
		 ...
		 {
			if($('#entry_559721990').val()!=''){
				var url = base_url + 'ajax_landlord.php'
				$.ajax({
					type: 'post',
					url: url,
					data: {'action':'ajaxgetlandlord','landlord_mobile':$('#entry_559721990').val()}, 
					success: function(response) {
						jsonData = $.parseJSON(response);
						console.log(jsonData);
						...
						if(jsonData[0] == 'block'){
							...
							$('#entry_559721990').css('border','1px solid #55595c;');
							$('#559721990_error').hide();
							$("#ss-submit-landlord").removeAttr('disabled');
							$("#entry_748538924").val(jsonData['email']);
							$('#entry_748538924').prop('readonly', true);
			...
			LANDLORD_EXISTS   = $("#landlordexists").val();
            ...
            if(PHONE == ''){
				$('#entry_559721990').css('border','2px solid red');
				$('#559721990_error').html('Required Field');
				$('html, body').animate({ scrollTop: $("#entry_559721990").offset().top }, 1000);
                return;
			}
			else{
