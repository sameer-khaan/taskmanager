...
//landlord Rental Form end
        ...
        $("body").on("blur", "#entry_559721990", function(e){
			if($(this).val()!=''){
				...
				$.ajax({
					type: 'post',
					url: url,
					data: {'action':'ajaxgetlandlord','landlord_mobile':$(this).val()}, 
					success: function(response) {
						...
						if(jsonData[0] == 'block'){
							...
							$('#559721990_error').text('This number is blocked')
							$('#559721990_error').show();
							$("#ss-submit-landlord").prop('disabled', true);
						}
						else if(jsonData[0] == 'empty'){
							$("#ss-submit-landlord").removeAttr('disabled');
							$("#ss-submit-landlord").text('Register & Submit');	
							$('#entry_748538924').prop('readonly', false);
							$("#landlordexists").val(0);
						}
						else if(jsonData[0] == 'data'){
							$(this).css('border','1px solid #55595c;');
							$('#559721990_error').hide();
							$("#ss-submit-landlord").removeAttr('disabled');
							$("#entry_748538924").val(jsonData['email']);
							$('#entry_748538924').prop('readonly', true);
							$("#entry_1479665104").val(jsonData['name']);
							$("#landlord_id").val(jsonData['landid']);
							if(jsonData['landid'] == '0'){
								$("#landlordexists").val(0);
							}else{
								$("#landlordexists").val(1);
							}
						}
					}
				}); 
        ...
        $("body").on("blur", "#initials", function(e){
         ...
         $("#sub").click(function(e){
            ...
            $.ajax({
                    url: base_url+'landlord/inc/loginRegisterHandler.php?action=ajax_landlord_login',
                    ...
                    success: function(response)
                    {
						...
						console.log(res);
						if(res[0]==1){
							$('#entry_748538924').val(username);
							$('#landlord_id').val(res[1]);
							$("#ss-submit-landlord").text('Submit');	
							$('#landlogin').modal('hide'); 	
							$("#ss-submit-landlord").trigger('click');
							return false;							
						}else{
							$('#verierror').html(res[0]);
						}
                    }
            });			 
		 });
         ...
         $("#ss-submit-landlord").click(function(e)
		 {
			...
			$("#username").val(EMAIL);
			var LANDLORD_EXISTS = $("#landlordexists").val();
			...
			if(AGENT_OR_LANDLORD && NAME && PHONE && EMAIL && ADDRESS && LAT && LON && ZIP && BEDROOMS && 
               ...
               PETS && FLEX_CREDIT && UNIT_NO && AVAILABLE && INITIALS) {
                ...
                postData	+=	'&landlordexists='+LANDLORD_EXISTS;
                postData	+=	'&action=newrecord';
				$('#usernameotp').val(EMAIL);
                $.ajax({
                        url: base_url+'view/customer_request.php',
                        data: postData,
                        type: 'post',
						beforeSend:function(xhr, status, error){
							$("#ss-submit-landlord").html("<img src='"+base_url+"assets/img/processing.gif' width='20px' height='20px'> Please Wait...");
						},
                        error: function(xhr, status, error)
                        {
							$("#ss-submit-landlord").html('Submit');
                            console.log('status: '+status);
                            console.log('error: '+error);
                        ...
                        success: function(response)
                        {
                            ...
                            if(jsonData[1] == 'success')
							{
                                ...
                                if(FILES != ''){
									...
									if(LANDLORD_EXISTS==0){
										var url = base_url + 'landlord/inc/ajax.php'; 
										$.ajax({
											type: "POST",
											url : url, 
											data:  {"contact_info": PHONE,"action": "genotp"},
											success: function(reply){ 
												var arrrep = reply.split('--');
												$('#landlord_id_otp').val(arrrep[2]);
											}
										});
										$('#landloginotp').modal('show'); 				
										return false;
									}else{
										if($('#landlord_verified').val()==1){
											$('#landlogin').modal('show'); 
										}else{
											$('#landloginotp').modal('show'); 	
										}
									}
		...
		$('#landlogin btn-default').click(function(e){  
			$("#ss-submit-landlord").prop('value', 'Submit');
		});	
		
		$('#subotp').click(function(e){  
			var usernameotp = $.trim($('#usernameotp').val());
			var passwordotp = $.trim($('#passwordotp').val());
			var userid = $('#landlord_id_otp').val();
			var key_valueotp = $('#otptxt').val();
			if(usernameotp.length==0){
				$('#usernameotp').css('border','1px solid red');
				return false;
			}
			if(passwordotp.length==0){
				$('#passwordotp').css('border','1px solid red');
				return false;
			}else{
				$('#passwordotp').css('border','1px solid rgba(0,0,0,.15)');
			}			
			if(key_valueotp.length==0){
				$('#otptxt').css('border','1px solid red');
				return false;
			}else{
				$('#passwordotp').css('border','1px solid rgba(0,0,0,.15)');
			}			
			if(key_valueotp.length==6){
				var url = base_url + 'landlord/inc/ajax.php'; 
				$.ajax({
					type: "POST",
					url : url, 
					data:  {"otp_val": key_valueotp,"userid": userid,'action':'otp'} ,
					success: function(reply){ 
						if(reply==9){
							$('#559721990_error').html('Please enter valid OTP code');
							$('#559721990_error').css('color','red');
							$('#559721990_error').show();
						}else if(reply==1){

							$.ajax({
									url: base_url+'landlord/inc/loginRegisterHandler.php?action=ajax_landlord_login',
									data: {"username": usernameotp, "password": passwordotp},
									type: 'post',
									success: function(response)
									{
										var res = response.split('--');
										...
										if(res[0]==1){
											$('#verierrorotp').html('');
											var urldashboard = base_url+'agentteam/landlord_dashboard.php';
											window.setTimeout(function(){ window.location = urldashboard; }, 5000);
										}else{
											$('#verierrorotp').html(res[0]);
										}
									}
							});
						}
			...
			}else if(key_value.length<6 || key_value.length>6){
				$('#559721990_error').html('Please enter 6 digit OTP code');
				$('#559721990_error').css('color','red');
				$('#559721990_error').show();
			}			
		});
 		$("#resendotp").on("click", function(ev){	
				var PHONE = $.trim($('#entry_559721990').val());
				var url = base_url + 'landlord/inc/ajax.php'; 
				$.ajax({
				type: "POST",
				url : url, 
				...
				success: function(reply){ 
					var arrrep = reply.split('--');
					if(arrrep[0]==0 && arrrep[1]==2){
						$('#landlord_id_otp').val(arrrep[2]);
					}else{
						$('#559721990_error').html('Try again');
						$('#559721990_error').css('display','block');						
					}
				}
				});	
		});
		...
		//used in view/landlord.php
		 $("#ss-submit").click(function(e)
		 {
            ...
            EMAIL	         =  $.trim($('#entry_748538924').val());
			UNIT_SELLING     =  $.trim($('#entry_140706325').val());
			landlordexists     =  $.trim($("#landlordexists").val());
			LANDLORD          = $.trim($('#landlord_id').val());
			...
			if(CALLER && AGENT_OR_LANDLORD && NAME && PHONE && ADDRESS && LAT && LON && ZIP && BEDROOMS && RENT_COST && 
               SECURITY_COST && MOVEIN_COST && UTILITIES && FLOOR && PETS && FLEX_CREDIT) {
				...
				postData	+=	'&flex_credit='+FLEX_CREDIT;
				postData	+=	'&landlord_email='+EMAIL;
				postData	+=	'&landlordexists='+landlordexists;
				postData	+=	'&action=newrecord';
				if(LANDLORD != ''){
                    postData    +=  '&landlordid='+LANDLORD;
                }				
