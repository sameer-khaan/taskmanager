 jQuery(document).ready(function() {
	 ...
	 $("#current_previous_employer_current, #current_previous_employer_previous").change(function(e){
		...
		else
		{
			...
			postData	=	"pdf_user_id="+pdf_user_id+"&current_previous_employer="+current_previous_employer+'&company1=&from4=NULL&to4=NULL&to4_current=NULL&add4=&phone4=&position2=&netincome2=';
   ...
   if($('#current_previous_employer_val').length)
   {
	   ...
	   {
		    //-----mohit-20-may-2017-removing-double-click-from-checkbox------------//
		   $('#current_previous_employer_'+currentpreviousemployer).trigger('click');
	...
	//----------------------co-applicant/co-signer check-uncheck----------------------------------------//
	 $("#co_applicant_signer_applicant, #co_applicant_signer_signer").change(function(e){
		...
		else
		{
			...
			postData	=	"pdf_user_id="+pdf_user_id+"&co_applicant_signer="+co_applicant_signer+'&username2=&dob2=NULL&ssn2=&add5=&coApplicantSign1=&coApplicantSign2=&coApplicantSign3=&coApplicantSign4=&coApplicantSign5=&coApplicantSign6=&coApplicantSign7=';
   ...
   if($('#co_applicant_signer_val').length)
   {
	   ...
	   {
		   //-----mohit-20-may-2017-removing-double-click-from-checkbox------------//
		   $('#co_applicant_signer_'+coapplicantsigner).trigger('click');
   ...
   //----------------------co-applicant/co-signer check-uncheck----------------------------------------//
	 ...
	 $("#current_previous_employer2_current, #current_previous_employer2_previous").change(function(e){
		...
		else
		{
			...
			postData	=	"pdf_user_id="+pdf_user_id+"&current_previous_employer2="+current_previous_employer2+'&company2=&from8=NULL&to8=NULL&to8_current=NULL&add8=&phone8=&position4=&netincome4=';
   ...
   if($('#current_previous_employer2_val').length)
   {
	   ...
	   {
		    //-----mohit-20-may-2017-removing-double-click-from-checkbox------------//
		   $('#current_previous_employer2_'+currentpreviousemployer2val).trigger('click');
		   /*
   ...
   $('#applicant_form_submit').click(function(){
	   ...
	   //-----------------check co-applicant co-signer------------------------//
		...
		$( ".requiredField" ).each(function() 
						{
							...
							if($( this ).text() == 'Empty' || $( this ).text() == 'empty' || $( this ).text() == 'Initials' || $( this ).text() == 'initials')
							{
								...
								//$(this).next().remove();
								if($(this).next().hasClass('requiredError'))
								{
									$(this).next().remove();
								}
		...
		if(flag == 1)
		{
			...
			{
				alert("Error: Please refresh the page and check fields again.");
				return false;
			}
...
function updatePdfFastTrackFields()
{
	...
	onbehalfofcompany1	=	$.trim($('#onbehalfofcompany1').text());
	r_zips				=	'';
	...
	if(onbehalfofcompany1 == '' || onbehalfofcompany1 == 'empty' || onbehalfofcompany1 == 'Empty')
	{
		onbehalfofcompany1	=	'';
	}
	
	if(username == '' || username == 'empty' || username == 'Empty')
	{
		username	=	'';
	}
	...
	postData	=	"pdf_user_id="+pdf_user_id+"&username="+username+"&r_bedrooms="+r_bedrooms+"&r_people="+r_people+"&r_rent="+r_rent+"&r_chicago="+r_chicago+"&r_suburbs="+r_suburbs+"&r_cat="+r_cat+"&r_dog="+r_dog+"&r_if_dog="+r_if_dog+"&r_text1="+r_text1+"&r_text2="+r_text2+"&r_zips="+r_zips+"&onbehalfofcompany1="+onbehalfofcompany1;
